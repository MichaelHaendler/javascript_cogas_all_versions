...
// ItemHolder.prototype.contains = function(item,mX,mY){
	...
	// var temp  = "(mX > this.ULX && mX < this.URX) is: " + (mX > this.ULX && mX < this.URX);
	// printWords.AddToList(temp);	
	
	// var temp  = "(mY > this.ULY && mY < this.LLY) is: " + (mY > this.ULY && mY < this.LLY);
	// printWords.AddToList(temp);	
	...
	// var containsItem = 	(
		// ((item.getULX() < this.ULX && item.getURX() > this.ULX) && (item.getLRY() > this.URY && item.getLRY() < this.LRY)) ||
		// ((item.getULX() < this.URX && item.getURX() > this.URX) && (item.getLRY() > this.URY && item.getLRY() < this.LRY))
						// );
						
	// var temp  = "containsMouse is: " + containsMouse;
	// printWords.AddToList(temp);	

	// var temp  = "containsItem is: " + containsItem;
	// printWords.AddToList(temp);			
						
	// if(containsMouse && containsItem){
		// printWords.AddToList("item AND mouse are in the item column!!!!");
		// return true;
	// }
	// else{
		// return false;
...
// ItemHolder.prototype.contains = function(item,mX,mY){
	...
	// // var temp  = "this.ULX is: " + this.ULX;
	// // printWords.AddToList(temp);
	
	// // var temp  = "this.URX is: " + this.URX;
	// // printWords.AddToList(temp);
	...
	// // var temp  = "item.getLLX() is: " + item.getLLX();
	// // printWords.AddToList(temp);
	
	// // var temp  = "(item.getULX() > this.ULX && item.getULX() < this.URX) is: " + (item.getULX() > this.ULX && item.getULX() < this.URX);
	// // printWords.AddToList(temp);		
	
	// // var temp  = "(item.getURX() > this.ULX && item.getURX() < this.URX) is: " + (item.getURX() > this.ULX && item.getURX() < this.URX);
	// // printWords.AddToList(temp);
	
	// // var temp  = "(item.getULY() > this.ULY && item.getLLY() < this.LLY) is: " + (item.getULY() > this.ULY && item.getLLY() < this.LLY);
	// // printWords.AddToList(temp);
...
ItemHolder.prototype.set_new_coords_on_this_item = function(item){
	...
	item.setULX(items_new_ULX);
	
	//console.log("item.getULX now is: " + item.getULX());
	
	//item.setULX(600);
	
	item.setULY(items_new_ULY);
	
	item.setURX(items_new_URX);
	item.setURY(items_new_URY);
	
	item.setLLX(items_new_LLX);
	item.setLLY(items_new_LLY);
	
	item.setLRX(items_new_LRX);
	item.setLRY(items_new_LRY);
